(function() {
  ...
  randomIndex = function(array) {
    // return (rs.random() * 10000 | 0) % array.length;
  };
  randomRange = function(base, limit) {
    // return (rs.random() * 10000 | 0) % (limit - base) + base;
  };
  ...
  SVG = (function() {
    function SVG() {}
    SVG.prototype.renderFile = function(file, canvas, color, outcb) {
      var ctx, height, ignored, parser, svg, width;
      if (!outcb) { outcb = color; }
        ...
        cb.onStartElementNS(__bind(function(elem, attrs, prefix, uri, namespaces) {
          ...
          if (elem === 'svg') {
            ...
            if (style === 'fill:#A6A8AB;' || style === 'fill:#A5A6AA;') {
              _ref = color || randomColor(), r = _ref[0], g = _ref[1], b = _ref[2];
      ...
      for (_i = 0, _len = parts.length; _i < _len; _i++) {
        ...
        for (_j = 0, _len2 = args.length; _j < _len2; _j++) {
          ...
          if (arg && (arg != null)) {
            ...
            if (isNaN(parsed)) {
              console.error("invalid path data: " + part);
            }
            op.push(parsed);
          }
        }
